...
import { ResetPasswordRequest, UpdateProfileRequest } from "./types";
...
export const getUserProfile = createAsyncThunk( "getUserProfile/Profile", async (data: { email: string }, thunkAPI) => {
        thunkAPI.dispatch(updateLoading(1));
        try {
            const config = {
                headers: {
            ...
            const res = await axios.put(`/authentication/user-profile`, data, config);
...
export const resetPassword = createAsyncThunk( "resetPassword/Profile", async (data: ResetPasswordRequest, thunkAPI) => {
        thunkAPI.dispatch(updateLoading(1));
        try {
            const config = {
                headers: {
            ...
            const res = await axios.put(`/authentication/reset-password`, data, config);
            ...
            if(data.navigate) data.navigate("/logout")
